    ...
    def read_meta(self):
        ...
        if self.split != 'novel':
            ...
            for index, i in enumerate(img_list):
                ...
                # get sparse depth: prefer COLMAP points; fallback to external CSV if available
                depth = -torch.ones(H, W)
                dense_depth = depth_estimator.estimate_depth(img.cuda()).cpu()
                if self.split == "train":
                    used_colmap_sparse = False
                    if hasattr(self, 'depth_gts') and isinstance(self.depth_gts, list) and len(self.depth_gts) > 0:
                        img_name = os.path.basename(images_lis[i])
                        for depth_data in self.depth_gts:
                            if depth_data.get("name") == img_name:
                                for j in range(len(depth_data['coord'])):
                                    x = round(depth_data['coord'][j,1]); x = x if x < H else H-1
                                    y = round(depth_data['coord'][j,0]); y = y if y < W else W-1
                                    depth[x, y] = depth_data['depth'][j]
                                used_colmap_sparse = True
                                break
                    if not used_colmap_sparse:
                        try:
